...
type Store struct {
	mu       sync.RWMutex
	programs map[uint32]domain.ProgramMetadata
}

// New creates a new in-memory store.
func New() *Store {
	return &Store{
		programs: make(map[uint32]domain.ProgramMetadata),
	}
}
...
func (s *Store) Save(_ context.Context, kernelID uint32, metadata domain.ProgramMetadata) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) List(_ context.Context) (map[uint32]domain.ProgramMetadata, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()

	result := make(map[uint32]domain.ProgramMetadata, len(s.programs))
